...
def add_fast_rcnn_multilabel_losses(model):
    ...
    action_prob, loss_action = model.net.SoftmaxWithLoss(
        ['action_cls_logits', 'multilabels_int32'], ['action_prob', 'loss_action'],
        scale=model.GetLossScale()
    )
    loss_bbox = model.net.SmoothL1Loss(
        [
    ...
    loss_gradients = blob_utils.get_loss_gradients(model, [loss_action, loss_cls, loss_bbox])
    model.Accuracy(['cls_prob', 'labels_int32'], 'accuracy_cls')
    model.AddLosses(['loss_action', 'loss_cls', 'loss_bbox'])
    model.AddMetrics('accuracy_cls')
    return loss_gradients
