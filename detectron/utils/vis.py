...
def my_vis_one_image(
        ...
        kp_thresh=1, dpi=200, box_alpha=0.0, dataset=None, show_class=False,
        ext='pdf'):
    """Visual debugging of detections."""
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    ...
    output_name_kpts = os.path.basename(im_name) + '.kpts.npy'
    output_path_kpts = output_dir+'/'+output_name_kpts
    ...
    for i in sorted_inds:
        ...
        if classes[i]==1:
            ...
            detected_kpts.append([keypoints[i],score])
        ...
        if show_class:
            ax.text(
                ...
                bbox=dict(
                    facecolor='g', alpha=0.4, pad=0, edgecolor='none'),
                color='white')
        # print(classes[i]) classes[i]==1 -> [person]
    ...
    np.save(output_path_kpts,detected_kpts) 
    f.close()
    output_name = os.path.basename(im_name) + '.' + ext
    fig.savefig(os.path.join(output_dir, '{}'.format(output_name)), dpi=dpi)
    plt.close('all')
